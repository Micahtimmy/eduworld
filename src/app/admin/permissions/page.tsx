        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 28 }}>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            ...
            <button style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '9px 18px', background: T.primary, border: 'none', borderRadius: 10, fontSize: 13, color: '#fff', fontWeight: 600, cursor: 'pointer' }}>
              <span className="material-symbols-outlined" style={{ fontSize: 16 }}>person_add</span>New Admin Staff
            </button>
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
          ...
          <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 16, padding: 24 }}>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ borderBottom: `1px solid ${T.border}` }}>
                  <th style={{ textAlign: 'left', paddingBottom: 10, fontSize: 11, fontWeight: 700, color: T.textMuted, paddingRight: 16 }}>Permission</th>
                  {['Super Admin', 'Registrar', 'Finance', 'Dept. Head'].map(h => (
              ...
              <tbody>
                {PERMISSIONS.map((p, i) => (
                  <tr key={p.label} style={{ borderBottom: i < PERMISSIONS.length - 1 ? `1px solid ${T.border}` : 'none' }}>
                    <td style={{ padding: '12px 0', display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: T.textPrimary }}>
                      <span className="material-symbols-outlined" style={{ fontSize: 16, color: T.primary }}>{p.icon}</span>
                      {p.label}
                    </td>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button style={{ padding: '8px 16px', background: T.surface, border: `1px solid ${T.border}`, borderRadius: 8, fontSize: 13, color: T.textMuted, cursor: 'pointer' }}>Reset Default</button>
              <button style={{ padding: '8px 16px', background: T.primary, border: 'none', borderRadius: 8, fontSize: 13, color: '#fff', fontWeight: 600, cursor: 'pointer' }}>Save Matrix Changes</button>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={{ background: T.ai + '08', border: `1px solid ${T.ai}30`, borderRadius: 14, padding: 20 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 10 }}>
                ...
                <span style={{ fontSize: 14, fontWeight: 700, color: T.textPrimary }}>EduWorld AI: Access Anomaly Detected</span>
              </div>
              <p style={{ fontSize: 13, color: T.textMuted, marginBottom: 14 }}>
                Department Head <strong style={{ color: T.textPrimary }}>Linda Hsiung</strong> attempted to access <strong style={{ color: T.textPrimary }}>Financial Ledger</strong> twice in the last 24 hours. Permission denied. Consider reviewing role requirements.
              </p>
              <div style={{ display: 'flex', gap: 8 }}>
                <button style={{ flex: 1, padding: '8px 0', background: T.surface, border: `1px solid ${T.border}`, borderRadius: 8, fontSize: 13, color: T.textPrimary, cursor: 'pointer' }}>Review Attempt</button>
                <button style={{ flex: 1, padding: '8px 0', background: T.surface, border: `1px solid ${T.border}`, borderRadius: 8, fontSize: 13, color: T.textMuted, cursor: 'pointer' }}>Dismiss Insight</button>
              </div>
            </div>

            {/* Audit Log */}
            <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 16, padding: 24, flex: 1 }}>
              ...
              <p style={{ fontSize: 12, color: T.textMuted, marginBottom: 16 }}>Real-time administrative activity.</p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {AUDIT.map(a => (
                  <div key={a.label} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '10px 12px', background: T.bg, borderRadius: 10 }}>
                    <span className="material-symbols-outlined" style={{ fontSize: 16, color: a.color, flexShrink: 0 }}>{a.icon}</span>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontWeight: 600, fontSize: 13, color: T.textPrimary }}>{a.label}</div>
